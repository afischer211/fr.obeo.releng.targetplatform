...
class TargetPlatformValidator extends AbstractTargetPlatformValidator {
	...
	def checkOptionsOnLocationAreIdentical(TargetPlatform targetPlatform) {
		if (targetPlatform.options.empty) { // else do not check as it is another error.
			...
			val conflicts = listOptions.tail.filter[o| !Sets::symmetricDifference(o.options.toSet,first.options.toSet).empty]
			if (!conflicts.empty) {
				listOptions.forEach[o |
					val nodes = NodeModelUtils::findNodesForFeature(o, TargetPlatformPackage.Literals.LOCATION__OPTIONS)
					if (!nodes.empty) {
						val withKeyword = (nodes.head as CompositeNode).previousSibling
						val lastOption = (nodes.last as CompositeNode)
						acceptError("Options of every locations must be the same",
							o, withKeyword.offset, lastOption.endOffset - withKeyword.offset, CHECK__OPTIONS_EQUALS_ALL_LOCATIONS)
					} else {
						val node = NodeModelUtils::getNode(o)
						acceptError("Options of every locations must be the same",
							o, node.offset, node.length, CHECK__OPTIONS_EQUALS_ALL_LOCATIONS)
					}
				]
			}
		}
